...
"information is generated.\n"
  ...
  opt_reg_string(odb, "-cache:dl1",
		 ...
		 &cache_dl1_opt, "dl1:256:32:1:l:1", /* print */TRUE, NULL);
  opt_reg_note(odb,
"  The cache config parameter <config> has the following format:\n"
"\n"
"    <name>:<nsets>:<bsize>:<assoc>:<repl>:<use trail>\n"
"\n"
...
"                -dtlb dtlb:128:4096:32:r:0\n"
  ...
  opt_reg_string(odb, "-cache:dl2",
		 "l2 trail-0 data cache config, i.e., {<config>|none}",
		 &cache_dl2_opt, "dl2:128:64:4:l:0", /* print */TRUE, NULL);
  opt_reg_string(odb, "-cache:il1",
		 "l1 inst cache config, i.e., {<config>|dl1|dl23|none}",
...
"      -cache:dl1 dl1:256:32:1:l:1 -cache:dl2 ul2:512:64:2:l:1\n"
"\n"
"    Or, a fully unified cache hierarchy (il1 pointed at dl1):\n"
"      -cache:il1 dl1\n"
"      -cache:dl1 ul1:256:32:1:l:1 -cache:dl2 ul2:512:64:2:l:1\n"
	       );
  opt_reg_string(odb, "-cache:il2",
		 "l2 instruction cache config, i.e., {<config>|dl23|none}",
		 &cache_il2_opt, "il2:512:64:4:l:0", /* print */TRUE, NULL);
...
{
  char name[128], c;
  int nsets, bsize, assoc, use_trail;
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	fatal("the l1 data cache must be defined if the l2 cache is defined");
      cache_dl2 = NULL;
    }
  else /* dl1 is defined */
    {
      if (sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &use_trail) != 6)
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<usetrail>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       dl1_access_fn, /* hit latency */1, use_trail /*use trail */);
      /* use a level 2 trail 0 D-cache? */
      if (!mystricmp(cache_dl2_opt, "none"))
        cache_dl2 = NULL;
      	
      else /* the level 2 D-cache is defined */
      {
	  if (sscanf(cache_dl2_opt, "%[^:]:%d:%d:%d:%c:%d",
                 name, &nsets, &bsize, &assoc, &c, &use_trail) != 6)
          fatal("bad l20 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<usetrail>");
          cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
                               /* usize */0, assoc, cache_char2policy(c),
                               dl2_access_fn, /* hit latency */1,use_trail);
	  if(cache_dl2)
	  printf("D-L2 Created!\n");
      }
    }
  ...
  if (!mystricmp(cache_il1_opt, "none"))
    ...
    {
      if (!cache_dl2)
	fatal("I-cache l1 cannot access D-cache l20 as it's undefined");
      cache_il1 = cache_dl2;
  ...
  else /* il1 is defined */
    {
      if (sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &use_trail) != 6)
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<usetrail>");
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       il1_access_fn, /* hit latency */1, use_trail);
      ...
      else
	{
	  if (sscanf(cache_il2_opt, "%[^:]:%d:%d:%d:%c:%d",
	    ...
	    fatal("bad l2 I-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>:<usetrail>");
  ...
  if (!mystricmp(itlb_opt, "none"))
  ...
  else
    {
      if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &use_trail) != 6)
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>:<usetrail>");
		 ...
		 int nbytes)		/* number of bytes to access */
{
  ...
  if (cache_dl1){
    ...
    /*FILE *f;
    f = fopen("/home/ananda/SPM_ANALYSIS/perl.txt", "a");
